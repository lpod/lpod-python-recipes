...
of the image it finds in the document with the accessibility text provided.
 ...
 Nota : LPOD currently can't directly acces images in styles / header
"""
...
if  __name__ == '__main__':
    ...
    t0 = time.time()
    content_footprint = make_footprint(file(options.image).read())

    for source in sources:
        if os.path.isdir(source):
            for root, dirs, files in os.walk(source):
                for name in files:
                    set_infos(os.path.join(root, name),
                                     content_footprint,
                                     options.title,
                                     options.description)
        else:
