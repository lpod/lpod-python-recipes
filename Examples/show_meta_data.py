...
def get_default_doc():
    ...
    default_filename = "collection2.odt"

    # For convenience we use some remote access to the document
    #from urllib2 import urlopen
    #default_filename = urlopen("http://arsaperta.org/collection2.odt")
...
if __name__=="__main__":
    ...
    if meta.get_statistic() is not None:
        for key, value in meta.get_statistic().iteritems():
            print '   %-18s: %s' % (key[5:], value)
    if meta.get_user_defined_metadata() is not None:
        print 'user defined metadata'
        for key, value in meta.get_user_defined_metadata().iteritems():
